...
./luau-lsp analyze --sourcemap=sourcemap.json --defs=globalTypes.d.luau \
  --base-luaurc=.luaurc --platform=roblox $(find . -name '*.luau' | sort)
```
...
lo detectan: un umbral de sector escrito como texto (lo caza la pasada 1), un
material que no existe, un número suelto dentro de un sistema y un remote
declarado pero no creado (los caza la pasada 2). Una comprobación que nunca ha
fallado no es una comprobación.
